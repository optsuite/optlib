...
lemma one_iter (h₁ : ∀ x₁ : E, HasGradientAt f (f' x₁) x₁) (hfun: ConvexOn ℝ Set.univ f)
    ...
    ∀ (k : ℕ+), f (x k) - f xm - (1 - γ k) * (f (x (k - 1)) - f xm) ≤
    l * (γ k) ^ 2 / 2 * (‖v (k - 1) - xm‖ ^ 2 - ‖v k - xm‖ ^ 2)  := by
  have h2 : ∀ (k : ℕ+), ∀ x' : E , f (x k) - f x' ≤ l * inner (x k - y k) (x' - x k)
      + l / 2 * ‖x k - y k‖ ^ 2 := by
    intro k x'
    rw [sub_le_iff_le_add', ← add_assoc]
    have : (f' (y k)) = l • (y k - x k) := by
      specialize update2 k
      rw [eq_sub_iff_add_eq', ← eq_sub_iff_add_eq] at update2
      rw [← update2, smul_smul]
      field_simp [h₂]
    have t1 : f (y k) + inner (f' (y k)) (x' - y k) ≤ f x' := by
      exact first_order_condition' (h₁ (y k)) hfun (by trivial) x' (by trivial)
    calc
      f (x k) ≤ f (y k) + inner (f' (y k)) (x k - y k) + l / 2 * ‖x k - y k‖ ^ 2 := by
          exact lipschitz_continuos_upper_bound' h₁ h₃ (y k) (x k)
        _ = f (y k) + inner (f' (y k)) (x' - y k + (x k - x')) + l / 2 * ‖x k - y k‖ ^ 2 := by
          rw [add_comm (x' - y k), add_sub (x k - x'), sub_add, sub_self, sub_zero]
        _ = f (y k) + inner (f' (y k)) (x' - y k) + inner  (f' (y k)) (x k - x')
              + l / 2 * ‖x k - y k‖ ^ 2 := by rw [inner_add_right, ← add_assoc]
        _ ≤ f x' + inner  (f' (y k)) (x k - x') + l / 2 * ‖x k - y k‖ ^ 2 := by
  ...
  have h3 : ∀ (k : ℕ+), f (x k) - f xm - (1 - γ k) * (f (x (k - 1)) - f xm) ≤ l * (inner
    ...
    have : γ k • (xm - x k) + (1 - γ k) • (x (k - 1) - x k)
        = (1 - γ k) • (x (k - 1)) + ((γ k) • xm)- x k := by
    ...
    calc γ k * (f (x k) - f xm) + (1 - γ k) * (f (x k) - f (x (k - 1))) ≤
            γ k * (l * (inner (x k - y k) (xm - x k)) + l / 2 * ‖x k - y k‖ ^ 2) + (1 - γ k) *
            (l * (inner (x k - y k) (x (k - 1) - x k)) + l / 2 * ‖x k - y k‖ ^ 2) := by
              apply add_le_add
              · exact mul_le_mul_of_nonneg_left (h2 k xm) hz
              · exact mul_le_mul_of_nonneg_left (h2 k (x (k - 1))) (by linarith)
          _ = l * (γ k * (inner (x k - y k) (xm - x k))) + l * ((1 - γ k) *
                (inner (x k - y k) (x (k - 1) - x k))) +  l / 2 * ‖x k - y k‖ ^ 2 := by ring_nf
          _ = l * inner (x k - y k) (γ k • (xm - x k)) + l * (inner (x k - y k) ((1 - γ k) •
                 (x (k - 1) - x k))) +  l / 2 * ‖x k - y k‖ ^ 2 := by
              rw [← inner_smul_right _ _ (γ k), ← inner_smul_right _ _ (1 - γ k)]
          _ = l * inner (x k - y k) (γ k • (xm - x k) + (1 - γ k) • (x (k - 1) - x k))
                + l / 2 * ‖x k - y k‖ ^ 2 := by rw [← mul_add, ← inner_add_right (x k - y k)]
          _ = l * inner (x k - y k) ((1 - γ k) • (x (k - 1)) + ((γ k) • xm)- x k)
                + l / 2 * ‖x k - y k‖ ^ 2 := by rw [this]
  intro k
  have lzero:  0 < k + (1 : ℝ) := Nat.cast_add_one_pos k
  have hz : γ k ≥ (0 : ℝ) := by
    rw [hg k]
  ...
  have this1 : l * (γ k) ^ 2 / 2 * (‖v (k-1) - xm‖ ^ 2 - ‖v k - xm‖ ^ 2) =
      l / 2 * (‖y k - (1 - γ k) • (x (k - 1)) - γ k • xm‖ ^ 2 -
      ‖x k - (1 - γ k) • x (k - 1) - γ k • xm‖ ^ 2) := by
    calc l * (γ k) ^ 2 / 2 * (‖v (k-1) - xm‖ ^ 2 - ‖v k - xm‖ ^ 2) =
            l / 2 * ((γ k) ^ 2 * ‖v (k-1) - xm‖ ^ 2 - (γ k) ^ 2 * ‖v k - xm‖ ^ 2) := by ring_nf
        _ = l / 2 * ((‖γ k‖ * ‖(v (k-1) - xm)‖) ^ 2 - (‖γ k‖ * ‖(v k - xm)‖) ^ 2) := by
            simp; rw [abs_of_nonneg hz]; ring_nf; left; simp
        _ = l / 2 * (‖γ k • (v (k-1) - xm)‖ ^ 2 - ‖γ k • (v k - xm)‖ ^ 2) := by
            rw [norm_smul, norm_smul]
        _ = l / 2 * (‖y k - (1 - γ k) • (x (k - 1)) - γ k • xm‖ ^ 2 -
              ‖γ k • (x (k - 1) + (1 / (γ k)) • (x k - x (k - 1))) - γ k • xm‖ ^ 2) := by
