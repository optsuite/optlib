...
-- A bounded-above sequence satisfies hxα: inf is bounded above.
section
...
lemma α_subseq_converge {φ : ℕ → ℝ}
   (h : ∀ k : ℕ, φ k.succ ∈ Ioo (x - 1 / k.succ) (x + 1 / k.succ)) :
...
lemma add_BddAbove' (hAa : BddAbove (range A.x))
   (hBa : BddAbove (range B.x)) : BddAbove (range (A.x + B.x)) :=
  BddAbove.range_add hAa hBa
...
lemma add_le_sinf (hAa : BddAbove (range A.x)) (hBa : BddAbove (range B.x)) :
  ...
  rcases hb with ⟨n,hn⟩
  rw[← hn.2]
  apply add_le_add <;>apply sinf_le_mem
  · exact hn.1
  exact hn.1
...
lemma point_eventually {b : ℝ} {φ : ℕ → E} (h : ∀ᶠ a in 𝓝 x', f a > b)
    (hφ : Tendsto φ atTop (𝓝 x')) : ∀ᶠ n in atTop, f (φ n) > b := by
...
def comp_real_liminf : real_liminf :=
    bounded_liminf (IsBoundedUnder.bddBelow_range (f_IsBoundedUnder
      f lscf x' x x_converge)) (hfa)
...
lemma le_liminf_of_lowerSemicontinuous :
    lim_inf (comp_real_liminf f lscf x' x x_converge hfa) ≥ f x' := by
  by_contra! h
...
def tendsto_real_liminf (x_converge : Tendsto f atTop (𝓝 x')) : real_liminf := {
  ...
  hxα := BddAbove_inf_BddAbove (f_converge_BddBelow x' f x_converge)
    (Tendsto.bddAbove_range x_converge)
}
