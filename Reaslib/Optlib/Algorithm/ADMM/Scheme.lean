...
-- Basic form of problems solved by ADMM
...
-- ADMM subproblems have unique solutions
...
def Augmented_Lagrangian_Function (opt : OptProblem E₁ E₂ F) (ρ : ℝ) : E₁ × E₂ × F → ℝ :=
   fun (x₁ , x₂ , y) => (opt.f₁ x₁) + (opt.f₂ x₂)
      + inner ℝ y  ((opt.A₁ x₁) + (opt.A₂ x₂) - opt.b)
...
class ADMM extends (OptProblem E₁ E₂ F) where
   ...
   hrho : ρ > 0
   htau  : 0 < τ ∧ τ < ( 1 + √ 5 ) / 2
   itex₁ : ∀ k, IsMinOn (fun x₁ ↦ (Augmented_Lagrangian_Function E₁ E₂ F toOptProblem ρ)
      (x₁ , x₂ k , y k)) univ (x₁ ( k + 1 ))
   uitex₁ : ∀ k , Admm_sub_Isunique (fun x₁ ↦ (Augmented_Lagrangian_Function E₁ E₂ F toOptProblem ρ)
      (x₁ , x₂ k , y k)) (x₁ ( k + 1 )) (itex₁ k)
   itex₂ : ∀ k, IsMinOn (fun x₂ ↦ (Augmented_Lagrangian_Function E₁ E₂ F toOptProblem ρ)
      (x₁ (k+1) , x₂ , y k)) univ (x₂ ( k + 1 ))
   uitex₂ : ∀ k , Admm_sub_Isunique (fun x₂ ↦ (Augmented_Lagrangian_Function E₁ E₂ F toOptProblem ρ)
      (x₁ (k+1) , x₂ , y k)) (x₂ ( k + 1 )) (itex₂ k)
