...
lemma Φ_bd_above [Setting E₁ E₂ F admm admm_kkt] : ∃ C : ℝ, ∀ n : ℕ, Φ n < C := by
   ...
   | succ k h =>
      ...
      · push_neg at hh
        ...
        change Φ ((k.toPNat k_pos) + 1) < C
        linarith

--byf
-- Φ is a bounded sequence
lemma Φ_isBounded' [Setting E₁ E₂ F admm admm_kkt] : ∃ (r : ℝ), (range Φ) ⊆ ball 0 r := by
   ...
   rcases hx with ⟨n,eq⟩; rw [← eq, abs_eq_self.2]
   · exact bd n
   · apply Φ_is_nonneg

lemma Φ_isBounded [Setting E₁ E₂ F admm admm_kkt] :
    IsBounded (range Φ) := (isBounded_iff_subset_ball 0).2  Φ_isBounded'


--lhj
-- e_y is a bounded sequence
lemma ey_isBounded' [Setting E₁ E₂ F admm admm_kkt] : ∃ (r : ℝ), (range ey) ⊆ ball 0 r := by
   ...
   have open_Φ (n : ℕ) : τ * ρ * Φ n = ‖ey n‖^2 + τ * ρ * ρ * ‖A₂ (e₂ n)‖ ^ 2
      + τ * ρ * ((max (1 - τ) (1 - 1 / τ)) * ρ ) * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^2 := by
      simp [Φ, Ψ]; ring_nf; rw [mul_assoc, mul_assoc, mul_assoc];
      nth_rw 2 [← mul_assoc] ; rw [mul_inv_cancel₀, one_mul]
      rw [← mul_assoc, mul_inv_cancel₀, one_mul]; ring; apply ne_of_gt hτ; apply ne_of_gt hρ
   have open_Φ' (n : ℕ) :  ‖ey n‖^2 + τ * ρ * ρ * ‖A₂ (e₂ n)‖ ^ 2
      + τ * ρ * ((max (1 - τ) (1 - 1 / τ)) * ρ ) * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^2
      = ‖ey n‖^2 + (τ * ρ * ρ * ‖A₂ (e₂ n)‖ ^ 2
        + τ * ρ * ((max (1 - τ) (1 - 1 / τ)) * ρ ) * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^2) := by
      ring_nf
   have nonneg1 (n : ℕ): τ * ρ * ρ * ‖A₂ (e₂ n)‖ ^ 2 ≥ 0 := by
      ...
      apply pow_two_nonneg
   have nonneg2 (n : ℕ): τ * ρ * ((max (1 - τ) (1 - 1 / τ)) * ρ )
      * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^2 ≥ 0 := by
      ...
      apply mul_nonneg h3
      apply pow_two_nonneg
   have nonneg3 (n : ℕ): τ * ρ * ρ * ‖A₂ (e₂ n)‖ ^ 2
      + τ * ρ * ((max (1 - τ) (1 - 1 / τ)) * ρ ) * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^2 ≥ 0 := by
   ...
   have ey_norm_pow (n : ℕ) : ‖ey n‖ ^ 2 < τ * ρ * r := by
      have Φ_lt_r :  τ * ρ * Φ n < τ * ρ * r := by
        apply mul_lt_mul_of_pos_left (Φ_isBounded n) τ_mul_ρ'
      apply lt_of_le_of_lt (ey_norm n) Φ_lt_r
   have ey_norm_above (n : ℕ) : ‖ey n‖ < √(τ * ρ * r) := by
      have h_ey: √(‖ey n‖^2) = ‖ey n‖ := by
        rw [pow_two]; apply Real.sqrt_mul_self; apply norm_nonneg
      rw [← h_ey]
...
lemma ey_isBounded [Setting E₁ E₂ F admm admm_kkt] :
    IsBounded (range ey ) := (isBounded_iff_subset_ball 0).2  ey_isBounded'

--gyq
-- A₂ e₂ is a bounded sequence
lemma unfold_Φ [Setting E₁ E₂ F admm admm_kkt] :
    ∀ n, ‖Φ n‖ = ‖1 / (τ * ρ) * ‖ey n‖ ^ 2
+ ρ * ‖A₂ (e₂ n)‖ ^ 2
...
lemma max_tau_mul_nonneg [Setting E₁ E₂ F admm admm_kkt] :
    ∀ n, 0 ≤ max (1 - τ) (1 - 1 / τ) * ρ * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2 := by
   intro n
   apply mul_nonneg
   · apply mul_nonneg
...
lemma ineq1 [Setting E₁ E₂ F admm admm_kkt] : ∀ n, ρ * ‖A₂ (e₂ n)‖ ^ 2 ≤ ‖Φ n‖ := by
   ...
   have h_norm_eq : ‖x1 + x2 + x3‖ = x1 + x2 + x3 := by
      rw [Real.norm_of_nonneg];linarith [hx1, hx2, hx3]
...
lemma A₂e₂_isBounded' [Setting E₁ E₂ F admm admm_kkt] :
    ∃ (r : ℝ), (range (A₂ ∘ e₂) ) ⊆ ball 0 r := by
...
-- A₁ e₁ + A₂ e₂ is a bounded sequence
lemma Φ_sub_nonneg [Setting E₁ E₂ F admm admm_kkt] :
    ∀ n : ℕ+ , 0 ≤ (Φ n) - (Φ (n + 1)) := by
   intro n
...
lemma ineq2 [Setting E₁ E₂ F admm admm_kkt] :
    0 < min 1 (1 + 1 / τ - τ ) * ρ := by
   apply mul_pos nonneg₂ ADMM.hrho

lemma A₁e₁_A₂e₂_isBounded' [Setting E₁ E₂ F admm admm_kkt] :
    ∃ (r : ℝ), (range (A₁ ∘ e₁ + A₂ ∘ e₂) ) ⊆ ball 0 r := by
   -- obtain r_Φ
   have hΦ : ∃ r_Φ, range Φ ⊆ Metric.ball 0 r_Φ := by apply Φ_isBounded'
   rcases hΦ with ⟨r_Φ, hΦ⟩
   ...
   have h2 (k : ℕ+): (min τ (1 + τ - τ ^ 2) ) * ρ * ‖A₂ (x₂ k - x₂ (k + 1))‖ ^ 2
      + (min 1 (1 + 1 / τ - τ )) * ρ * ‖A₁ (e₁ (k + 1)) + A₂ (e₂ (k + 1))‖ ^ 2
      ≤ (Φ k) - (Φ (k + 1)) := by
      apply Φ_isdescending
   ...
   have h4 (k : ℕ+):
      ...
      calc (min 1 (1 + 1 / τ - τ )) * ρ * ‖A₁ (e₁ (k + 1)) + A₂ (e₂ (k + 1))‖ ^ 2
         _ ≤ (min τ (1 + τ - τ ^ 2) )* ρ * ‖A₂ (x₂ k - x₂ (k + 1))‖ ^ 2 +
             (min 1 (1 + 1 / τ - τ )) * ρ * ‖A₁ (e₁ (k + 1)) + A₂ (e₂ (k + 1))‖ ^ 2 := h1 k
         _ ≤ (Φ k) - (Φ (k + 1)) := h2 k
         _ < 2 * r_Φ := h3 k

   have h5 (k : ℕ+):
      ‖A₁ (e₁ (k + 1)) + A₂ (e₂ (k + 1))‖ <
      √ (2 * r_Φ / ((min 1 (1 + 1 / τ - τ )) * ρ)) := by
      have h4_ins := h4 k
      have h5a: 0 ≤ ‖A₁ (e₁ (k + 1)) + A₂ (e₂ (k + 1))‖ := by apply norm_nonneg
      calc ‖A₁ (e₁ (k + 1)) + A₂ (e₂ (k + 1))‖
         _ = √ ((‖A₁ (e₁ (k + 1)) + A₂ (e₂ (k + 1))‖) ^ 2) := by
            conv in ‖A₁ (e₁ (k + 1)) + A₂ (e₂ (k + 1))‖ => rw [← Real.sqrt_sq h5a]; rfl
   ...
   have h5' (n : ℕ) (hn : 1 < n):
      ‖A₁ (e₁ n) + A₂ (e₂ n)‖ < √ (2 * r_Φ / ((min 1 (1 + 1 / τ - τ )) * ρ)) := by
   ...
   let r := (max (max (√ (2 * r_Φ / ((min 1 (1 + 1 / τ - τ )) * ρ)))
                       (‖A₁ (e₁ 0) + A₂ (e₂ 0)‖ + 1 ))
                  (‖A₁ (e₁ 1) + A₂ (e₂ 1)‖ + 1 ))
   have hr' : r = (max (max (√ (2 * r_Φ / ((min 1 (1 + 1 / τ - τ )) * ρ)))
                             (‖A₁ (e₁ 0) + A₂ (e₂ 0)‖ + 1 ))
                        (‖A₁ (e₁ 1) + A₂ (e₂ 1)‖ + 1 )) := by rfl
   use r
...
lemma A₁e₁_isBounded' [Setting E₁ E₂ F admm admm_kkt] :
   ...
   have h_ineq : ‖A₁ (e₁ n)‖ ≤ ‖A₁ (e₁ n) + A₂ (e₂ n)‖ + ‖A₂ (e₂ n)‖ := by
...
lemma x₁_isBounded' [Setting E₁ E₂ F admm admm_kkt] (fullrank₁ : Function.Injective admm.A₁) :
   ...
   calc
      _ = ‖(x₁ n - x₁') + x₁'‖ := by
         rw [add_comm, add_sub, add_comm, add_sub_assoc, sub_self, add_zero]
...
-- limit point of the subsequence
local notation "x₁''" => Converge_Subseq.x₁'' (Subseq fullrank₁ fullrank₂)
local notation "x₂''" => Converge_Subseq.x₂'' (Subseq fullrank₁ fullrank₂)
local notation "y''"  => Converge_Subseq.y'' (Subseq fullrank₁ fullrank₂)

-- subsequence map is strictly increasing
lemma hphi_StrictMono [Setting E₁ E₂ F admm admm_kkt] : StrictMono φ :=
  (Subseq fullrank₁ fullrank₂).hphi


-- subsequence map is strictly increasing: (x₁ (φ n), x₂ (φ n), y (φ n)) → (x₁', x₂', y')
-- subsequence converges
--lim_{n → ∞} (uₙ ,vₙ ) = 0 -> lim_{n → ∞} uₙ  = 0
lemma admm_nhds_prod_eq [Setting E₁ E₂ F admm admm_kkt] :
    𝓝 (x₁'' , x₂'' , y'') = 𝓝 x₁'' ×ˢ 𝓝 x₂'' ×ˢ 𝓝 y'' := by
   rw[nhds_prod_eq,nhds_prod_eq]

lemma hconverge [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => (x₁ (φ n),  x₂ (φ n), y (φ n))) atTop (𝓝 x₁'' ×ˢ 𝓝 x₂'' ×ˢ 𝓝 y'') := by
   have := (Subseq fullrank₁ fullrank₂).hconverge
   rw[admm_nhds_prod_eq] at this
   exact this

lemma x₁_subseq_converge [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => (x₁ (φ n))) atTop (𝓝 x₁'') :=
...
lemma y_subseq_converge [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => (y (φ n))) atTop (𝓝 y'') :=
...
lemma φ₁_equ [Setting E₁ E₂ F admm admm_kkt] :
    ∀ n : ℕ , φ₁ n = φ (n + 1) := by
   intro n
   have : φ (n + 1) > 0 := by
      calc φ (n + 1)
...
lemma x₁_subseq_converge' [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => (x₁ (φ₁ n))) atTop (𝓝 x₁'') := by
...
lemma Φ_Summable₁' [Setting E₁ E₂ F admm admm_kkt] :
   ...
   have : ∀ i ∈ Finset.range n , (φ₀ i)-(φ₀ (i+1)) = (Φ i.succ ) - (Φ (i.succ + 1)) := by
      simp [Finset.mem_range, Nat.succ_eq_add_one]
      exact fun i hi ↦ by rfl
...
lemma Φ_isSummable [Setting E₁ E₂ F admm admm_kkt] :
    Summable (fun n : ℕ => (Φ (n+1) ) - (Φ (n+1 + 1))) := by
...
lemma Summable₁' [Setting E₁ E₂ F admm admm_kkt] :
   ...
   simp only
   let m := (n+1).toPNat'
   change (min 1 (1 + 1 / τ - τ )) * ρ * ‖A₁ (e₁ (m+1)) + A₂ (e₂ (m+1))‖ ^ 2 ≤ Φ m - Φ (m + 1)
   apply Φ_inequ₁

lemma Summable₁ [Setting E₁ E₂ F admm admm_kkt] :
    Summable (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖^2) := by
   let φn := fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2
   let φnsucc := fun n => (φn (n+2))
   change Summable φn
   rw[← summable_nat_add_iff 2]
   change Summable (fun n => φnsucc n)
...
lemma Summable₂' [Setting E₁ E₂ F admm admm_kkt] :
   ...
   simp only
   let m := (n+1).toPNat'
   change (min τ (1 + τ - τ ^ 2)) * ρ * ‖A₂ (x₂ (m + 1) - x₂ m)‖^2 ≤ Φ m - Φ (m + 1)
   apply Φ_inequ₂

-- variable [Setting E₁ E₂ F admm admm_kkt]
lemma Summable₂ [Setting E₁ E₂ F admm admm_kkt] :
    Summable (fun n => ‖A₂ (x₂ (n + 1) - x₂ n)‖^2) := by
...
-- byf
lemma y_subseq_converge' [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => (y (φ₁ n))) atTop (𝓝 y'') := by
...
-- byf
lemma square_converge_zero₁ [Setting E₁ E₂ F admm admm_kkt]
    (h : Tendsto (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2) atTop (𝓝 0)) :
    Tendsto (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖) atTop (𝓝 0) := by
   have : Tendsto (fun n => √((‖A₁ (e₁ n) + A₂ (e₂ n)‖)^2)) atTop (𝓝 √0) := by
      apply Filter.Tendsto.sqrt h
   rw [Real.sqrt_zero] at this; simp at this; exact this

-- byf
-- ‖A₁ (e₁ n) + A₂ (e₂ n)‖ → 0
lemma converge_zero₁ [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖) atTop (𝓝 0) := by
   have : Tendsto (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2) atTop (𝓝 0) := by
...
-- byf
-- with the square norm of A₂ (x₂ (n + 1) - x₂ n) → 0,
-- we can infer that the norm of A₂ (x₂ (n + 1) - x₂ n) also → 0
lemma square_converge_zero₃ [Setting E₁ E₂ F admm admm_kkt]
    (h : Tendsto (fun n => ‖A₂ (x₂ (n + 1) - x₂ n)‖ ^ 2) atTop (𝓝 0)) :
...
lemma Seq_converge_zero₁ [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => A₁ (e₁ n) + A₂ (e₂ n)) atTop (𝓝 0) := by
...
-- byf
-- A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)) → 0
lemma sub_Seq_converge_zero₁ [Setting E₁ E₂ F admm admm_kkt] :
...
-- A₁ (x₁ (φ₁ n)) + A₂ (x₂ (φ₁ n)) - b → 0
lemma sub_Seq_converge_zero₂ [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => A₁ (x₁ (φ₁ n)) + A₂ (x₂ (φ₁ n)) - b) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converge_zero₂
   apply StrictMono.tendsto_atTop
   have : (fun n => (Int.toNat (φ₁ n))) = (fun n => (φ (n+1))) := by
      ext n; rw [φ₁_equ fullrank₁ fullrank₂ n]; simp
...
-- A₂ (x₂ ((φ₁ n) + 1) - x₂ (φ₁ n)) → 0
lemma sub_Seq_converge_zero₃ [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => A₂ (x₂ ((φ₁ n) + 1) - x₂ (φ₁ n))) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converge_zero₃
   apply StrictMono.tendsto_atTop
   have : (fun n => (Int.toNat (φ₁ n))) = (fun n => (φ (n+1))) := by
      ext n; rw [φ₁_equ fullrank₁ fullrank₂ n]; simp
...
-- A₂ (x₂ (φ₁ n) - x₂ ((φ₁ n) - 1)) → 0
lemma sub_Seq_converge_zero₃' [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => A₂ (x₂ (φ₁ n) - x₂ ((φ₁ n) - 1))) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converge_zero₃
   apply StrictMono.tendsto_atTop
...
lemma const_smul_subseq₁ [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => (( 1 - τ) * ρ ) • (A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)))) atTop (𝓝 0) := by
   rw [← smul_zero (( 1 - τ) * ρ )]
...
-- ρ • A₂ (x₂ ((φ₁ n) - 1) - x₂ (φ₁ n)) → 0
lemma const_smul_subseq₂ [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => ρ • A₂ (x₂ ((φ₁ n) - 1) - x₂ (φ₁ n))) atTop (𝓝 0) := by
   have : (fun n => ρ • A₂ (x₂ ((φ₁ n) - 1) - x₂ (φ₁ n))) =
      (fun n => (-ρ) • A₂ (x₂ (φ₁ n) - x₂ ((φ₁ n) - 1))) := by
      ext n
      calc
...
lemma u_subseq_converge [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => u (φ₁ n)) atTop (𝓝 (- A₁† y'')) := by
   ...
   have : Tendsto (fun n => (y (φ₁ n)) + (( 1 - τ) * ρ ) • (A₁ (e₁ (φ₁ n))
      ...
      rw [← add_zero y'']
      apply Filter.Tendsto.add (y_subseq_converge' fullrank₁ fullrank₂)
        (const_smul_subseq₁ fullrank₁ fullrank₂)
...
lemma v_subseq_converge [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => v (φ₁ n)) atTop (𝓝 (- A₂† y'')) := by
   have : (fun n => v (φ₁ n)) =
      (fun n => - A₂† (y (φ₁ n) + (( 1 - τ) * ρ ) • (A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n))))) := by
      funext n; rw [v]
   rw [this]
   have h: Tendsto (fun n => (y (φ₁ n) + (( 1 - τ) * ρ ) • (A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)))))
      atTop (𝓝 y'') := by
      rw [← add_zero y'']
      apply Filter.Tendsto.add (y_subseq_converge' fullrank₁ fullrank₂)
        (const_smul_subseq₁ fullrank₁ fullrank₂)
...
lemma A₂'y_inthesubgradient [Setting E₁ E₂ F admm admm_kkt] :
    - A₂† y'' ∈ SubderivAt f₂ x₂'' :=
...
lemma Satisfying_equational_constraint' [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => ‖A₁ (x₁ (φ n)) + A₂ (x₂ (φ n)) - b‖) atTop
    (𝓝 ‖(A₁ x₁'') + (A₂ x₂'') - admm.b‖) := by
   apply Tendsto.norm
...
lemma subconverge_zero₂ [Setting E₁ E₂ F admm admm_kkt] :
    Tendsto (fun n => ‖A₁ (x₁ (φ n)) + A₂ (x₂ (φ n)) - b‖) atTop (𝓝 0) := by
...
-- byf
-- A₁ (e₁ (φ n)) → 0
lemma A₁e₁_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
...
-- ‖A₂ (e₂ (φ n))‖ → 0
lemma nrm_A₂e₂_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
    Tendsto (fun n => ‖A₂ (e₂ (φ n))‖) atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.1; apply A₂e₂_subseq_converge_zero

-- ‖A₂ (e₂ (φ n))‖^2 → 0
lemma sqnrm_A₂e₂_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
    Tendsto (fun n => ‖A₂ (e₂ (φ n))‖^2) atTop (𝓝 0) := by
   rw [← zero_pow]
   · apply Filter.Tendsto.pow
     · apply nrm_A₂e₂_subseq_converge_zero
   · linarith

-- ρ * ‖A₂ (e₂ (φ n))‖^2 → 0
lemma const_sqnrm_A₂e₂_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
...
-- byf
-- A₁ (e₁ (φ n)) + A₂ (e₂ (φ n)) → 0
lemma A₁e₁_A₂e₂_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
    Tendsto (fun n => A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))) atTop (𝓝 0) := by
   rw [← add_zero 0]
   apply Tendsto.add (A₁e₁_subseq_converge_zero fullrank₁ fullrank₂)
     (A₂e₂_subseq_converge_zero fullrank₁ fullrank₂)

-- ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖ → 0
lemma nrm_A₁e₁_A₂e₂_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
    Tendsto (fun n => ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖) atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.1; apply A₁e₁_A₂e₂_subseq_converge_zero

-- ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖^2 → 0
lemma sqnrm_A₁e₁_A₂e₂_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
...
lemma const_sqnrm_A₁e₁_A₂e₂_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
    Tendsto (fun n => ((max (1-τ) (1-(1/τ)))*ρ) * ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖^2) atTop
      (𝓝 0) := by
   rw [← mul_zero]
   apply Filter.Tendsto.const_mul ((max (1-τ) (1-(1/τ)))*ρ)
     (sqnrm_A₁e₁_A₂e₂_subseq_converge_zero fullrank₁ fullrank₂)

-- byf
-- Φ (φ n) → 0
lemma Φ_subseq_converge_zero
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
    Tendsto (fun n => Φ (φ n)) atTop (𝓝 0) := by
   have h_add: Tendsto
       (fun n => (1 / (τ * ρ)) * ‖ey (φ n)‖^2 + ρ * ‖A₂ (e₂ (φ n))‖^2) atTop (𝓝 0) := by
      rw [← zero_add 0]
      apply Tendsto.add (const_sqnrm_ey_subseq_converge_zero fullrank₁ fullrank₂)
        (const_sqnrm_A₂e₂_subseq_converge_zero fullrank₁ fullrank₂)
   have h_add': Tendsto (fun n => (1 / (τ * ρ)) * ‖ey (φ n)‖^2 + ρ * ‖A₂ (e₂ (φ n))‖^2 +
       ((max (1-τ) (1-(1/τ)))*ρ) * ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖^2) atTop (𝓝 0) := by
      rw [← zero_add 0]
      apply Tendsto.add h_add (const_sqnrm_A₁e₁_A₂e₂_subseq_converge_zero fullrank₁ fullrank₂)
   have : (fun n => Φ (φ n)) = (fun n => (1 / (τ * ρ)) * ‖ey (φ n)‖^2 +
       ρ * ‖A₂ (e₂ (φ n))‖^2 +
...
lemma Φ_le
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
    ∀ n : ℕ , (⨅ i, (fun n ↦ Φ (n + 1)) i) ≤ Φ (φ n.succ) := by
   intro n
...
lemma Φ_converge_zero''
    [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] :
    Tendsto (fun _ : ℕ ↦ (⨅ i, (fun n ↦ Φ (n + 1)) i)) atTop
      (𝓝 (⨅ i, (fun n ↦ Φ (n + 1)) i)) := by
...
lemma A₂e₂_le_Φ (n : ℕ)
   ...
   have open_Φ (n : ℕ) :
      ρ⁻¹ * Φ n = ((ρ⁻¹) ^ 2) * τ⁻¹ * ‖ey n‖^2 + ‖A₂ (e₂ n)‖ ^ 2 +
      (max (1 - τ) (1 - 1 / τ)) * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2 := by
      simp [Φ, Ψ]; ring_nf; rw [mul_inv_cancel₀, one_mul, one_mul]; ring_nf
      apply ne_of_gt (admm.hrho)
   have open_Φ' (n : ℕ) :
      ((ρ⁻¹) ^ 2) * τ⁻¹ * ‖ey n‖^2 + ‖A₂ (e₂ n)‖ ^ 2 +
      (max (1 - τ) (1 - 1 / τ)) * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2 =
      ‖A₂ (e₂ n)‖ ^ 2 + (((ρ⁻¹) ^ 2) * τ⁻¹ * ‖ey n‖^2 +
      (max (1 - τ) (1 - 1 / τ)) * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2) := by
      ring_nf
   ...
   have nonneg3 (n : ℕ) :
      ((ρ⁻¹) ^ 2) * τ⁻¹ * ‖ey n‖^2 +
      (max (1 - τ) (1 - 1 / τ)) * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2 ≥ 0 := by
      apply add_nonneg (nonneg1 n) (nonneg2 n)
   rw [h1, open_Φ, open_Φ']; linarith [nonneg3 n]

lemma A₂e₂_le_Φ'
    [Setting E₁ E₂ F admm
      (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))] (n : ℕ) :
    ‖‖A₂ (e₂ n)‖ ^ 2‖ ≤ (1 / ρ) * Φ n := by
