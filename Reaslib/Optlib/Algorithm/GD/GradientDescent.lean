...
lemma mono_sum_prop (mono : ∀ k : ℕ, f (g (k + 1)) ≤ f (g k)):
  ...
  | succ j _ =>
    calc f (g (j + 1 + 1)) - f xm
            ≤ (Finset.range (j.succ + 1)).sum
              (fun (k : ℕ) ↦ f (g (k + 1))) / (j.succ + 1) - f xm := by
                linarith [mono_sum_prop_primal' mono j]
      _ = ((Finset.range (j.succ + 1)).sum (fun (k : ℕ) ↦ f (g (k + 1))) - f xm * (j.succ + 1))
            / (j.succ + 1) := by field_simp
      _ = (Finset.range (j.succ + 1)).sum (fun (k : ℕ) ↦ f (g (k + 1)) - f xm) / (j.succ + 1) := by
...
lemma convex_lipschitz (h₁ : ∀ x₁ : E, HasGradientAt f (f' x₁) x₁)
  ...
  intro x
  calc
    _ ≤ f x + inner ℝ (f' x) (x - a • (f' x) - x) + l / 2 * ‖x - a • (f' x) - x‖ ^ 2 :=
  ...
  have descent: ∀ x : E, f (x - alg.a • (f' x)) ≤
    ...
    have t2 : inner ℝ (f' x) (x - xm) - alg.a / 2 * ‖f' x‖ ^ 2 =
      ...
      calc
        ...
        _ = inner ℝ (f' x) (x - xm) - alg.a / (2 : ℝ)
            * ‖f' x‖ ^ 2 := by ring_nf; simp; left; rw [pow_two,mul_self_mul_inv alg.a]
    calc
        _ ≤ f x - alg.a / 2 * ‖f' x‖ ^ 2 :=
          convex_lipschitz alg.diff this step₂ alg.step₁ alg.smooth x
        _ ≤ f xm + inner ℝ (f' x) (x - xm) - alg.a / 2 * ‖f' x‖ ^ 2 := by
          linarith [convex_function alg.diff hfun x xm]
        _ = _ := by rw [add_sub_assoc, t2]
    ...
    · specialize xdescent (j + 1)
      calc
        _ = (Finset.range (j + 1)).sum (fun (k : ℕ) ↦ f (alg.x (k + 1)) - f xm)
          ...
          rw [Nat.succ_eq_add_one j]; rw [add_assoc j, one_add_one_eq_two]; ring
