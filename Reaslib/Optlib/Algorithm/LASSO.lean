...
lemma real_inner_eq_dot (x y : EuclideanSpace ℝ (Fin m)) : @inner ℝ _ _ x y = x ⬝ᵥ y := by
...
private lemma linear_gradient : ∀ x : (EuclideanSpace ℝ (Fin n)),
  ...
  use ε; use εpos
  intro y _
  have eq1 : b ⬝ᵥ (A *ᵥ y) - b ⬝ᵥ (A *ᵥ x) = (Aᵀ *ᵥ b) ⬝ᵥ (y - x) := by
    rw [dot_mul_eq_transpose_mul_dot, dot_mul_eq_transpose_mul_dot, ← dotProduct_sub]
  rw [eq1]
  simp only [inner_sub_right]
  rw [real_inner_eq_dot, real_inner_eq_dot]
  rw [← dotProduct_sub, sub_self, norm_zero]
  apply mul_nonneg
  · linarith
  · apply norm_nonneg
...
lemma norm_one_convex : ConvexOn ℝ univ (fun x : (EuclideanSpace ℝ (Fin n)) => ‖x‖₁) := by
  ...
  · intro x _ y _ a b anneg bnneg _
    ...
    simp
    calc
      |a * x i + b * y i| ≤ |a * x i| + |b * y i| := by apply abs_add_le
      _ = a * |x i| + b * |y i| := by
...
theorem norm_one_proximal
  ...
  rw [← geqth]
  change prox_prop ((t * μ) • (fun (x : EuclideanSpace ℝ (Fin n)) => ‖x‖₁)) x xm
  have tμpos : 0 < t * μ := by
  ...
  intro y
  rw [real_inner_eq_dot]
  simp only [dotProduct]
  -- Show 1 / (t * μ) = μ⁻¹ * t⁻¹
  have inv_eq : (1 : ℝ) / (t * μ) = μ⁻¹ * t⁻¹ := by field_simp
  rw [inv_eq]
  -- Expand each term in the sum
  have expand_term : ∀ i, ((μ⁻¹ * t⁻¹) • (x - xm)) i * (y - xm) i =
                          (μ⁻¹ * t⁻¹) * (x i - xm i) * (y i - xm i) := by
    intro i; simp [mul_comm, mul_left_comm]
  simp only [expand_term]
  rw [← sum_add_distrib]
  apply sum_le_sum
  intro i _
  ...
  · rw [hxm]; simp
    ...
    calc
      μ⁻¹ * t⁻¹ * x i * y i ≤ μ⁻¹ * t⁻¹ * |x i * y i| := by
        rw [mul_assoc _ (x i), mul_le_mul_iff_of_pos_left]
        · apply le_abs_self
        · rw [← mul_inv, inv_pos]; apply mul_pos
          · linarith [μpos]
          · linarith [tpos]
      _ ≤ |y i| * μ⁻¹ * t⁻¹ * t * μ := by
  ...
  · rw [eq_ite_iff, or_iff_right] at abs_subg
    ...
    let sgnxm := sign (xm i)
    have aux_mem : sgnxm ∈ SubderivAt abs (xm i) := by
      rw [abs_subg]; rfl
    rw [← mem_SubderivAt, HasSubgradientAt] at aux_mem
    specialize aux_mem (y i)
    have aux : |y i| ≥ |xm i| + sgnxm * (y i - xm i) := by
      have : inner ℝ sgnxm (y i - xm i) = sgnxm * (y i - xm i) := by
        rw [real_inner_comm]; rfl
      rw [this] at aux_mem
      exact aux_mem
    have aux2 : sgnxm * (y i - xm i) = μ⁻¹ * t⁻¹ * (x i - xm i) * (y i - xm i) := by
      simp only [sgnxm]
      ...
      · have eq2 : sign (sign (x i) * (|x i| - t * μ)) = 1 := by
          ...
          linarith [ieq]
        rw [eq2]; left; exact (Real.sign_of_pos hx).symm
      · have xneg : x i < 0 := by
        ...
        have eq2 : sign (sign (x i) * (|x i| - t * μ)) = -1 := by
          ...
          calc
            ...
            _ < 0 := by linarith
          linarith [ieq]
        rw [eq2]; left; exact (Real.sign_of_neg xneg).symm
      linarith [μpos, tpos]
    rw [aux2] at aux
    have : ‖xm i‖ = |xm i| := by simp [Real.norm_eq_abs]
    have : ‖y i‖ = |y i| := by simp [Real.norm_eq_abs]
    linarith [aux]
...
structure LASSO (A : Matrix (Fin m) (Fin n) ℝ) (b : (Fin m) → ℝ) (μ : ℝ)
    (μpos : 0 < μ) (Ane0 : A ≠ 0) (x₀ : (EuclideanSpace ℝ (Fin n))) where
...
instance {A : Matrix (Fin m) (Fin n) ℝ} {b : (Fin m) → ℝ} {μ : ℝ} {μpos : 0 < μ} {Ane0 : A ≠ 0}
  ...
  hconv := by
    rw [p.heq]; apply ConvexOn.smul
    · linarith [μpos]
    · apply norm_one_convex
    ...
    · intro y ydist
      ...
      calc
        ...
        _ ≤ |μ| * (n * (ε / n / μ)) := by
          ...
          · simp; linarith [μpos]
        _ = ε := by
          have : |μ| = μ := by rw [abs_of_pos μpos]
          rw [this]
          field_simp
