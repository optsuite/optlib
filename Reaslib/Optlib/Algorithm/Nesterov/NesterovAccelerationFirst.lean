...
theorem Nesterov_first_converge (minφ : IsMinOn (f + h) univ xm) :
      ...
      _ ≤ f y + ⟪f' y, x - y⟫ + 1 / (2 * alg.t k) * ‖x - y‖ ^ 2 := by
        ...
        calc (alg.l : ℝ) / 2
          ...
          _ ≤ (1 / 2) * (1 / alg.t k) := by
            ...
            · have : alg.t k ≤ 1 / (alg.l : ℝ) := (alg.tbound k).2
              rw [le_one_div (by linarith [(alg.tbound k).1]) alg.hl] at this
              exact this
            · norm_num
          _ = 1 / (2 * alg.t k) := by ring
        apply sq_nonneg
  ...
  have φieq3 (k : ℕ) : φ (alg.x (k + 1)) - φ xm - (1 - alg.γ k) * (φ (alg.x k) - φ xm) ≤
    ...
    calc
      ...
      _ ≤ (alg.γ k) * ((1 / alg.t k) • ⟪alg.x (k + 1) - alg.y k, xm - alg.x (k + 1)⟫ +
          ...
          1 / (2 * alg.t k) * ‖alg.x (k + 1) - alg.y k‖ ^ 2) := by
        apply add_le_add
        · have hpos : 0 < alg.γ k := (alg.γbound k).1
          apply mul_le_mul_of_nonneg_left ieq2 (le_of_lt hpos)
  ...
  have φieq4 (k : ℕ+) : φ (alg.x (k + 1)) - φ xm - (1 - alg.γ k) * (φ (alg.x k) - φ xm) ≤
    ...
    calc
      ...
      _ = (alg.γ k) ^ 2 / (2 * alg.t k) * (‖v k - xm‖ ^ 2 - ‖v (k + 1) - xm‖ ^ 2) := by
        rw [eq k, add_sub_cancel_left, ← smul_sub, mul_sub, mul_sub, norm_smul, mul_pow,
          norm_eq_abs]
  ...
  have cond' : ∀ n : ℕ+, (1 - alg.γ n) * α n ≤ α (n - 1) := by
    ...
    let cond := alg.cond n
    simp [α]
    rw [mul_div_assoc, mul_div_assoc, ← mul_assoc, mul_comm _ 2, mul_assoc]
    rw [mul_le_mul_iff_right₀]
    · rw [← mul_div_assoc]; exact cond
    · simp
  have h10 (n : ℕ) : α n * (alg.γ n ^ (2 : ℕ) / (2 * alg.t n)) = 1 := by
    simp only [α, pow_two]
    have ht : alg.t n ≠ 0 := by linarith [(alg.tbound n).1]
  ...
  calc
    f (alg.x (k + 1)) + h (alg.x (k + 1)) - f xm - h xm =
      ...
      rw [sub_sub, ← mul_assoc, mul_comm _ (α k), h10 k]
      simp only [φ, one_mul]
    _ ≤ alg.γ k ^ 2 / (2 * alg.t k) * nr k := by
      have hpos : 0 < alg.γ k ^ 2 / (2 * alg.t k) := by
        apply div_pos; rw [sq_pos_iff]; linarith [(alg.γbound k).1]; linarith [alg.tbound k]
      apply mul_le_mul_of_nonneg_left _ (le_of_lt hpos)
      simp [nr]
    _ ≤ alg.γ k ^ 2 / (2 * alg.t k) * nr 0 := by
      have hpos : 0 < alg.γ k ^ 2 / (2 * alg.t k) := by
        apply div_pos; rw [sq_pos_iff]; linarith [(alg.γbound k).1]; linarith [alg.tbound k]
      apply mul_le_mul_of_nonneg_left bound (le_of_lt hpos)
    _ ≤ alg.γ k ^ 2 / (2 * alg.t k) * ‖x0 - xm‖ ^ 2 := by
      ...
      calc
        ...
        _ = ‖alg.x 0 - xm‖ ^ 2 := by
          ...
          have ht_ne : alg.t 0 ≠ 0 := by linarith [alg.tbound 0]
          rw [mul_inv_cancel₀ ht_ne, one_mul, one_mul, alg.oriy, norm_sub_rev (alg.x 1) xm]
...
instance {f h : E → ℝ} {f' : E → E} {x0 : E} [p : Nesterov_first_fix_stepsize f h f' x0] :
  ...
  cond := by
    intro n
    simp only [p.teq n, p.teq (n - 1), p.γeq n, p.γeq (n - 1)]
    have hl_pos : (0 : ℝ) < p.l := p.hl
    have hl_ne : (p.l : ℝ) ≠ 0 := ne_of_gt hl_pos
    field_simp [hl_ne]
    -- Goal: (2 + ↑↑n) * (2 + ↑↑n - 2) ≤ (2 + ↑(↑n - 1)) ^ 2
    have hn : 1 ≤ (n : ℕ) := n.2
    calc (2 + (n : ℝ)) * (2 + (n : ℝ) - 2)
        = (2 + (n : ℝ)) * (n : ℝ) := by ring
      _ = 2 * (n : ℝ) + (n : ℝ) ^ 2 := by ring
      _ ≤ 1 + 2 * (n : ℝ) + (n : ℝ) ^ 2 := by linarith
      _ = (1 + (n : ℝ)) ^ 2 := by ring
      _ = (↑(↑n - 1 : ℕ) + 2) ^ 2 := by
...
theorem Nesterov_first_fix_stepsize_converge (minφ : IsMinOn (f + h) univ xm) :
  ...
  calc
    ...
    _ ≤ 2 * alg.l / (k + 2) ^ 2 * ‖x0 - xm‖ ^ 2 := by
      ...
      · rw [alg.γeq k, alg.teq k]
        -- Goal: (2 / (2 + k))² / (2 * (1 / l)) ≤ 2 * l / (k + 2)²
        -- After field_simp and ring_nf, both sides simplify to the same expression
        field_simp
        ring_nf
        -- Goal is now: l * 4 + l * k * 4 + l * k² ≤ l * 4 + l * k * 4 + l * k²
        rfl
      · apply sq_nonneg
