...
theorem Nesterov_second_convergence (minφ : IsMinOn (f + h) Set.univ xm) :
  ...
  have φdef : ∀ z : E, φ z = f z + h z := fun z ↦ rfl
  have h1 : ∀ k : ℕ+, alg.γ k • (alg.y (k - 1) - alg.y k) - alg.t k • (f' (alg.z k))
    ...
    have upd2 := @SubderivAt.pos_smul _ _ _
      ((alg.t k / alg.γ k) • h) (alg.y k) (alg.γ k) (alg.γbound k).1
  ...
  have cvxineq : ∀ k : ℕ+,
      h (alg.x k) ≤ (1 - alg.γ k) * h (alg.x (k - 1)) + alg.γ k * h (alg.y k) := by
  ...
  have hieq6 : ∀ k : ℕ+, f (alg.x k) ≤ f (alg.z k) +
      ⟪f' (alg.z k), (1 - alg.γ k) • alg.x (k - 1) + alg.γ k • alg.y k - alg.z k⟫ +
...
instance {f h : E → ℝ} {f' : E → E} {x0 : E} [p : Nesterov_second_fix_stepsize f h f' x0] :
  ...
  initial := p.initial
  cond := by
    intro n; rw [p.teq n, p.teq (n + 1), p.γeq n, p.γeq (n + 1)]; simp; field_simp [p.hl]
    ring_nf
...
theorem Nesterov_second_fix_stepsize_converge (minφ : IsMinOn (f + h) Set.univ xm) :
  ...
  calc
    ...
    _ ≤ 2 * alg.l / (k + 2) ^ 2 * ‖x0 - xm‖ ^ 2 := by
      ...
      rw [alg.γeq (k + 1), alg.teq (k + 1)]; simp; field_simp
      linarith
