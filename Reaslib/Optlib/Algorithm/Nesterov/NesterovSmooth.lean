  ...
  have h2 : ∀ (k : ℕ+), ∀ x' : E , f (alg.x k) - f x' ≤ alg.l *
    ...
    calc
      _ ≤ f (alg.y k) + inner ℝ (f' (alg.y k)) (alg.x k - alg.y k) +
        ...
        _ = f (alg.y k) + inner ℝ (f' (alg.y k)) (x' - alg.y k) +
            inner ℝ (f' (alg.y k)) (alg.x k - x') + alg.l / 2 * ‖alg.x k - alg.y k‖ ^ 2 := by
          rw [inner_add_right, ← add_assoc]
...
theorem nesterov_algorithm_smooth (hfun : ConvexOn ℝ Set.univ f)
  ...
  have h5 : ∀ (k : ℕ+), 1 / (γ k) ^ 2 * (f (alg.x k) - f xm) + alg.l.1 / 2 * ‖alg.v k - xm‖ ^ 2
    ...
    specialize h4 k
    specialize con k
    have γk_sq_pos : (γ k) ^ 2 > 0 := by
      rw [hg k]
      simp only [div_pow, gt_iff_lt]
      apply div_pos (by linarith)
      apply sq_pos_of_ne_zero
      exact Nat.cast_add_one_ne_zero ↑k
    have h4' : (f (alg.x k) - f xm) / (γ k) ^ 2 -
        (1 - γ k) * (f (alg.x (k - 1)) - f xm) / (γ k) ^ 2 ≤
          alg.l.1 / 2 * (‖alg.v (k-1) - xm‖ ^ 2 - ‖alg.v k -xm‖ ^ 2) := by
      rw [← sub_div]
      have two_pos : (0 : ℝ) < 2 := by norm_num
      have h4_mul : (f (alg.x k) - f xm - (1 - γ k) * (f (alg.x (k - 1)) - f xm)) * 2 ≤
          alg.l.1 * (γ k) ^ 2 * (‖alg.v (k-1) - xm‖ ^ 2 - ‖alg.v k -xm‖ ^ 2) := by
        have := mul_le_mul_of_nonneg_right h4 (le_of_lt two_pos)
        calc (f (alg.x k) - f xm - (1 - γ k) * (f (alg.x (k - 1)) - f xm)) * 2
            ≤ (alg.l.1 * γ k ^ 2 / 2 * (‖alg.v (k-1) - xm‖ ^ 2 - ‖alg.v k -xm‖ ^ 2)) * 2 := this
          _ = alg.l.1 * (γ k) ^ 2 * (‖alg.v (k-1) - xm‖ ^ 2 - ‖alg.v k -xm‖ ^ 2) := by ring
      calc (f (alg.x k) - f xm - (1 - γ k) * (f (alg.x (k - 1)) - f xm)) / (γ k) ^ 2
          = ((f (alg.x k) - f xm - (1 - γ k) * (f (alg.x (k - 1)) - f xm)) * 2)
            / ((γ k) ^ 2 * 2) := by ring
        _ ≤ (alg.l.1 * (γ k) ^ 2 * (‖alg.v (k-1) - xm‖ ^ 2 - ‖alg.v k -xm‖ ^ 2))
            / ((γ k) ^ 2 * 2) := by
              apply div_le_div_of_nonneg_right h4_mul; linarith [γk_sq_pos]
        _ = alg.l.1 / 2 * (‖alg.v (k-1) - xm‖ ^ 2 - ‖alg.v k -xm‖ ^ 2) := by
            have γk_ne : γ k ≠ 0 := by
              intro h; rw [h] at γk_sq_pos; linarith
            field_simp [γk_ne]
    have h4'' : 1 / (γ k) ^ 2 * (f (alg.x k) - f xm) + alg.l.1 / 2 * ‖alg.v k - xm‖ ^ 2 ≤
        (1 - γ k) / (γ k) ^ 2 * (f (alg.x (k - 1)) - f xm) +
          alg.l.1 / 2 * ‖alg.v (k-1) - xm‖ ^ 2 := by
      calc 1 / (γ k) ^ 2 * (f (alg.x k) - f xm) + alg.l.1 / 2 * ‖alg.v k - xm‖ ^ 2
          = (f (alg.x k) - f xm) / (γ k) ^ 2 + alg.l.1 / 2 * ‖alg.v k - xm‖ ^ 2 := by ring
        _ ≤ (1 - γ k) * (f (alg.x (k - 1)) - f xm) / (γ k) ^ 2 +
            alg.l.1 / 2 * ‖alg.v (k-1) - xm‖ ^ 2 := by linarith [h4']
        _ = (1 - γ k) / (γ k) ^ 2 * (f (alg.x (k - 1)) - f xm) +
            alg.l.1 / 2 * ‖alg.v (k-1) - xm‖ ^ 2 := by ring
    apply le_trans h4''
