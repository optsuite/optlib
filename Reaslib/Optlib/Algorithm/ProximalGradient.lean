...
theorem proximal_gradient_method_converge : ∀ (k : ℕ+),
  ...
  have univieq : ∀ x z : E,
    ...
    calc
      ...
      _ = φ z + inner ℝ (Gt x) (x - z) - alg.t / 2 * ‖Gt x‖ ^ 2 := by
        ...
        rw [this, sub_eq_add_neg, ← add_assoc, add_comm (h z) (f z)]
        unfold φ; ring
  ...
  have φdecrease' : ∀ i j : ℕ, (i ≤ j) → φ (alg.x j) ≤ φ (alg.x i) := by
    ...
    have diffφle : ∀ n : ℕ, diffφ n ≤ 0 := by
      intro n
      change φ (alg.x (n + 1 + i)) - φ (alg.x (n + i)) ≤ 0
  ...
  have ieq (k : ℕ) : k • (φ (alg.x k) - φ alg.xm) ≤
    ...
    nth_rw 1 [this];
    change s.card • (φ (alg.x k) - φ alg.xm) ≤ nr 0 - nr k
  ...
  have kpos : 0 < (k : ℝ) := Nat.cast_pos.mpr (PNat.pos k)
  have h2kpos : 0 < 2 * k * alg.t := by
    apply mul_pos; apply mul_pos; linarith; exact kpos; exact alg.tpos
  linarith
