...
theorem Lipschitz_to_bounded_subgradient (h : LipschitzWith G f) :
  ...
  have h₄' : f y - f x < ‖g‖:= by
    calc
      ...
      _ = ↑G := by
        rw [norm_div, norm_one, one_div, norm_norm]
        field_simp [hg₁]
      _ < ‖g‖ := by apply h₃
  linarith
...
theorem subgradient_method_converge :
  ...
  · simp [k₀]
    ...
    rw [mul_assoc, mul_assoc]; apply (mul_le_mul_iff_of_pos_left two_pos).mpr
    rw [mul_comm, ← mul_assoc]; apply (mul_le_mul_iff_of_pos_right (alg.ha 0)).mpr
  ...
  · have heq : (Set.range fun (x : Finset.range (k + 1)) => f (alg.x x)) =
    ...
    have h₁ :  ∀ ⦃i : ℕ⦄, i ≥ 0 ∧ i ≤ k → ‖alg.x (i+1) - xm‖ ^ 2 ≤ ‖alg.x i - xm‖ ^ 2 - 2 * alg.a i
      ...
      have : ‖alg.x i - xm‖ ^ 2 - 2 * alg.a i *
          (sInf {x | ∃ i ∈ Finset.range (k + 1), f (alg.x i) = x} - f xm) +
          ↑alg.G ^ 2 * alg.a i ^ 2 = ‖alg.x i - xm‖ ^ 2 +
          - (2 * alg.a i * (sInf {x | ∃ i ∈ Finset.range (k + 1), f (alg.x i) = x} - f xm)) +
          ↑alg.G ^ 2 * alg.a i ^ 2 := by ring
      rw [this]
      have inq₁: ‖alg.a i‖ ^ 2 * ‖alg.g i‖ ^ 2 ≤ ↑alg.G ^ 2 * alg.a i ^ 2 := by
        ...
        · rw [sq_le_sq]
          simp [abs_of_nonneg (norm_nonneg _), abs_of_nonneg (NNReal.coe_nonneg _)]
          exact hi
      have inq₂: 2 * alg.a i * (sInf {x | ∃ i ∈ Finset.range (k + 1), f (alg.x i) = x} - f xm)
          ≤ 2 * inner ℝ (alg.x i - xm) (alg.a i • alg.g i) := by
        rw [mul_assoc]; apply (mul_le_mul_iff_of_pos_left two_pos).mpr
        rw [inner_smul_right]; apply (mul_le_mul_iff_of_pos_left (alg.ha i)).mpr
...
theorem subgradient_method_fixed_distance {s : ℝ} (hm : IsMinOn f univ xm)
  ...
  have inq₁ : Finset.sum (Finset.range (k + 1)) (fun x => alg.a x) ≥ (k + 1) * (s / ↑alg.G) := by
    have : Finset.sum (range (k + 1)) (fun _ => s / (NNReal.toReal alg.G)) =
        (k + 1) * (s / ↑alg.G) := by
    ...
    intro i _
    rw [← (ha' i)]
    apply (div_le_iff₀ hG).mpr ((mul_le_mul_iff_of_pos_left (alg.ha i)).mpr (h' (alg.hg i)))
  ...
  calc
    ...
    _ ≤ (‖x₀ - xm‖ ^ 2 - ‖alg.x (k + 1) - xm‖ ^ 2 + (k + 1) * s ^ 2)
        / (2 * (k + 1) * (s / alg.G)) := by
          ...
          · intro x₂ hx₂; simp [s]
            calc
              ...
              _ < (ε / (2 * ↑alg.G ^ 2) + ε / (2 * ↑alg.G ^ 2)) / 2 := by
                apply (mul_lt_mul_iff_of_pos_left zero_lt_two).mp
          ...
          _ < ε / 4 + ε / 4 := by
            ...
            · rcases hasA b hba₁ with h₃; simp [s₁] at h₃
              ...
              calc
                ...
                _ < ε / 4 := by
                  apply (div_lt_iff₀ zero_lt_four).mpr
                  calc
                    _ = (2 * ↑alg.G ^ 2 * Finset.sum (Finset.range (a₂ + 1)) fun x => alg.a x ^ 2) /
                          Finset.sum (Finset.range (b + 1)) alg.a := by ring
                    _ < ε := h₃₂
                    _ = (ε / 4) * 4 := by ring
            · apply (div_lt_iff₀ hpos).mpr
              calc
                _ ≤ ↑alg.G ^ 2 * Finset.sum (Finset.range (b - a₂)) (fun x => (ε / (2 * ↑alg.G ^ 2))
                      * alg.a (a₂ + 1 + x)) := by
                    apply (mul_le_mul_iff_of_pos_left hpos').mpr; apply Finset.sum_le_sum; intro i _
                    have hposi : alg.a (a₂ + 1 + i) > 0 := by apply (alg.ha (a₂ + 1 + i))
                    rw [pow_two]; apply (mul_le_mul_iff_of_pos_right hposi).mpr
                    have : a₂ + 1 + i ≥ a₂ := by
                ...
                _ < ↑alg.G ^ 2 *  (ε / (2 * ↑alg.G ^ 2)) * Finset.sum (Finset.range (b + 1))
                      (fun x => alg.a x) := by
                    apply (mul_lt_mul_iff_of_pos_left hposG).mpr
