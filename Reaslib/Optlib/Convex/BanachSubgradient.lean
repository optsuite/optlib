...
theorem Banach_SubderivWithinAt.Nonempty (hf : ConvexOn ℝ s f)
  ...
  let h := - (1 / u) • g.1
  have g1_bounded : ∃ (C : ℝ), ∀ (y : E), |g.1 y| ≤ C * ‖y‖ := by
    have g2_zero : g.2 0 = 0 := by simp [hu]
    have g1_eq_phi : ∀ y, g.1 y = φ (y, 0) := by
      intro y
      have := hg (y, 0)
      simp [g2_zero] at this
      exact this.symm
    use ‖φ‖
    intro y
    rw [g1_eq_phi]
    calc
      |φ (y, 0)| = ‖φ (y, 0)‖ := rfl
      _ ≤ ‖φ‖ * ‖(y, (0 : ℝ))‖ := φ.le_opNorm (y, 0)
      _ = ‖φ‖ * ‖y‖ := by simp only [Prod.norm_def, norm_zero, max_eq_left (norm_nonneg y)]
  obtain ⟨C, hC⟩ := g1_bounded
  have hh : ∃ (C : ℝ), ∀ (x : E), ‖h x‖ ≤ C * ‖x‖ := by
    use (1 / u) * C
    intro x
    have h_def : h x = -(1 / u) * (g.1 x) := by
      simp [h, LinearMap.smul_apply, smul_eq_mul]
    rw [h_def, norm_mul, norm_neg]
    calc ‖1 / u‖ * |g.1 x|
      _ = (1 / u) * |g.1 x| := by simp [abs_of_pos hu0]
      _ ≤ (1 / u) * (C * ‖x‖) := by
        apply mul_le_mul_of_nonneg_left (hC x)
        exact div_nonneg zero_le_one (le_of_lt hu0)
      _ = ((1 / u) * C) * ‖x‖ := by ring
  let h' := (LinearMap.mkContinuousOfExistsBound h hh)
  have key1 : ∀ a ∈ interior (Epi f s) , h' (a.1 - x) + f x < a.2 := by
    intro a ha
    have sep_ineq : φ (x, f x) < φ a := hφ a ha
    rw [hg (x, f x), hg a] at sep_ineq
    simp only [hu] at sep_ineq
    have rearranged : g.1 x - g.1 a.1 < u * (a.2 - f x) := by
      linarith [sep_ineq]
    have linear_rewrite : g.1 x - g.1 a.1 = -g.1 (a.1 - x) := by
      rw [← LinearMap.map_sub]
      simp
    rw [linear_rewrite] at rearranged
    have step1 : g.1 (a.1 - x) > u * (f x - a.2) := by
      linarith [rearranged]
    have step2 : (1 / u) * g.1 (a.1 - x) > f x - a.2 := by
      have pos_inv_u : (1 / u) > 0 := by simp [hu0]
      have : (1 / u) * g.1 (a.1 - x) > (1 / u) * (u * (f x - a.2)) := by
        exact mul_lt_mul_of_pos_left step1 pos_inv_u
      have simplify : (1 / u) * (u * (f x - a.2)) = f x - a.2 := by
        rw [← mul_assoc, one_div_mul_cancel (ne_of_gt hu0), one_mul]
      rwa [simplify] at this
    have h_eq : h' (a.1 - x) = -(1 / u) * g.1 (a.1 - x) := by
      simp [h', LinearMap.mkContinuousOfExistsBound_apply, h, LinearMap.smul_apply, smul_eq_mul]
    rw [h_eq]
    have step3 : -(1 / u) * g.1 (a.1 - x) < a.2 - f x := by
      have : -(1 / u) * g.1 (a.1 - x) = -((1 / u) * g.1 (a.1 - x)) := by ring
      rw [this]
      have neg_ineq : -((1 / u) * g.1 (a.1 - x)) < -(f x - a.2) := neg_lt_neg step2
      have eq_rearrange : -(f x - a.2) = a.2 - f x := by ring
      rwa [eq_rearrange] at neg_ineq
    linarith [step3]
