...
lemma cone_trans (s : Finset ℕ) (V : ℕ → (EuclideanSpace ℝ (Fin n))) :
    cone' s (V ∘ coe s) = cone s V := by
  ...
  · simp; intro t tnneg xdecompose
    ...
    · have eq : ∀ i : s, t i • V i = t' i.val • V i := by
        intro i; simp [t']
      have eq' : (Finset.sum (attach s) fun x ↦ t' x • V x) =
          (Finset.sum (attach s) fun x ↦ t x • V x) := by
...
lemma cone_subset_of_idx_subset' (s τ : Finset ℕ) (hsub : τ ⊆ s)
    (V : ℕ → (EuclideanSpace ℝ (Fin n))) : cone τ V ⊆ cone s V := by
...
lemma closed_conic_idp (s : Finset ℕ) (V : s → (EuclideanSpace ℝ (Fin n)))
  ...
  have iscF : Continuous f := by
    ...
    let h := fun z : ℝ ↦ z • V i
    have : fi = h ∘ g := by rfl
    suffices Continuous (h ∘ g) by exact this
...
theorem closed_conic (s : Finset ℕ) (V : ℕ → (EuclideanSpace ℝ (Fin n))) : IsClosed (cone s V) := by
  ...
  rw [← cone_trans]
  let V₀ := (V ∘ coe τ)
  suffices IsClosed (cone' τ V₀) by exact this
  apply closed_conic_idp
  simp [V₀]; apply τidp
