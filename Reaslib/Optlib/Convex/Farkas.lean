...
lemma polyhedra_is_closed {σ : Finset ℕ} : ∀ (b : ℕ → EuclideanSpace ℝ (Fin n)),
    IsClosed {z | ∃ (mu : σ → ℝ), (∀ i, 0 ≤ mu i) ∧ z =
...
private lemma decompose_pn : ∀ (lam : τ → ℝ), ∃ (lamp lamn : ℕ → ℝ),
  ...
  intro lam
  let lamp : ℕ → ℝ := fun j => if h : (j ∈ τ)
    then if _ : (0 ≤ lam ⟨j, h⟩) then lam ⟨j, h⟩ else 0 else 0
  let lamn : ℕ → ℝ := fun j => if h : (j ∈ τ)
    then if _ : (0 ≤ lam ⟨j, h⟩) then 0 else -lam ⟨j, h⟩ else 0
  use lamp; use lamn
  intro i
...
private lemma shift_sum (τ : Finset ℕ) (m : ℕ) (f : ℕ → EuclideanSpace ℝ (Fin n)) :
  ...
  suffices (∑ i : τ, f i) = (∑ i : (Finset.image h τ), g i) by exact this
  simp
...
lemma general_polyhedra_is_polyhedra_empty (τ σ : Finset ℕ) (he : ¬(τ ∪ σ).Nonempty) :
    ...
    cone μ c := by
  simp only [not_nonempty_iff_eq_empty] at he; rw [Finset.union_eq_empty] at he
  intro a b; simp only [Subtype.forall, he, notMem_empty, forall_false, implies_true,
    isEmpty_subtype, not_false_eq_true, univ_eq_empty, sum_empty, add_zero, true_and,
    exists_const, Set.setOf_eq_eq_singleton]
  use ∅; use (fun _ => 0)
...
theorem Farkas :
    ...
    have : inner (ℝ) c z ≥ (0 : ℝ) := by
      calc
        _ = inner (ℝ) (Finset.sum univ (fun i ↦ lam i • a i)) z
            + inner (ℝ) (Finset.sum univ (fun i ↦ mu i • b i)) z := by
  ...
  have scon : Convex ℝ S := by
    ...
    use (a1 • tx + b1 • ty)
    constructor
    · suffices 0 ≤ (a1 • tx + b1 • ty) by exact this
