...
lemma Bounded_of_UpperBounded (hf : ConvexOn ℝ (ball x₀ r) f)
  ...
  · calc
      ...
      _ <  -f y + 2 * f x₀ := by
        ...
        rw[← mul_neg]
        simp only [Nat.ofNat_pos, mul_lt_mul_iff_right₀, neg_add']
        calc
...
lemma LocallyUpperBounded (hs_convex : Convex ℝ s) (hs_isopen : IsOpen s)
  ...
  have hx₁_in_hull : x₁ ∈ (convexHull ℝ) (↑u') := by
    simp[u']
    apply t_pos hx₁.1
  have sup_ub : ∀ y ∈ u', f y ≤ Finset.sup' u' u_nonempty f := fun y hy => Finset.le_sup' f hy
  rw [Finset.convexHull_eq] at hx₁_in_hull
  obtain ⟨w, hw_pos, hw_sum, hx₁_eq⟩ := hx₁_in_hull
  rw [← hx₁_eq, Finset.centerMass]
  have w_sum_pos : 0 < ∑ i ∈ u', w i := by rw [hw_sum]; norm_num
  simp only [id_eq]
  -- Jensen's inequality: f(∑ wᵢxᵢ) ≤ ∑ wᵢf(xᵢ) for convex combinations
  have key : f ((∑ j ∈ u', w j)⁻¹ • (∑ i ∈ u', w i • i)) ≤
             (∑ j ∈ u', w j)⁻¹ * (∑ i ∈ u', w i * f i) := by
    have : (∑ j ∈ u', w j) = 1 := hw_sum
    rw [this, inv_one, one_smul, one_mul]
    classical
    suffices ∀ (s : Finset α) (w : α → ℝ),
        (∀ i ∈ s, 0 ≤ w i) → (∑ i ∈ s, w i = 1) → (∀ i ∈ s, i ∈ (convexHull ℝ) ↑u') →
        f (∑ i ∈ s, w i • i) ≤ ∑ i ∈ s, w i * f i by
    ...
    · intro a t hat IH w hw_pos' hw_sum' hmem'
      ...
      have hw_t : ∀ i ∈ t, 0 ≤ w i := fun i hi => hw_pos' i (Finset.mem_insert_of_mem hi)
      by_cases ha : w a = 1
      · have hw_sum_insert : w a + ∑ j ∈ t, w j = 1 := by
          rw [← Finset.sum_insert hat]; exact hw_sum'
        have : ∀ i ∈ t, w i = 0 := by
          intro i hi; rw [ha] at hw_sum_insert
          have sum_nonneg : 0 ≤ ∑ j ∈ t, w j := Finset.sum_nonneg hw_t
          have sum_eq : ∑ j ∈ t, w j = 0 := by linarith
          have : w i ≤ ∑ j ∈ t, w j := Finset.single_le_sum hw_t hi
          linarith [hw_t i hi]
        have sum_zero : ∑ x ∈ t, w x • x = 0 := by
          apply Finset.sum_eq_zero; intro i hi; rw [this i hi]; simp
        have sum_f_zero : ∑ x ∈ t, w x * f x = 0 := by
          apply Finset.sum_eq_zero; intro i hi; rw [this i hi]; simp
        rw [sum_zero, sum_f_zero, ha]; simp
      · have wa_pos : 0 ≤ w a := hw_pos' a (Finset.mem_insert_self a t)
        have hw_sum_insert : w a + ∑ i ∈ t, w i = 1 := by
          rw [← Finset.sum_insert hat]; exact hw_sum'
        have wa_lt_1 : w a < 1 := by
          by_contra h; push_neg at h
          have : w a = 1 := by
            have sum_nonneg : 0 ≤ ∑ i ∈ t, w i := Finset.sum_nonneg hw_t
            have : 1 = w a + ∑ i ∈ t, w i := hw_sum_insert.symm; linarith
          contradiction
        have wt_pos : 0 < 1 - w a := by linarith
        by_cases ht_empty : t = ∅
        · simp [ht_empty] at hw_sum_insert; contradiction
        · let w' : α → ℝ := fun i => w i / (1 - w a)
          have hw'_pos : ∀ i ∈ t, 0 ≤ w' i := fun i hi =>
            div_nonneg (hw_t i hi) (le_of_lt wt_pos)
          have hw'_sum : ∑ i ∈ t, w' i = 1 := by
            simp [w', ← Finset.sum_div]; field_simp; linarith
          have hmem'_t : ∀ i ∈ t, i ∈ (convexHull ℝ) ↑u' := fun i hi =>
            hmem' i (Finset.mem_insert_of_mem hi)
          have IH' := IH w' hw'_pos hw'_sum hmem'_t
          have sum_eq : ∑ i ∈ t, w i • i = (1 - w a) • ∑ i ∈ t, w' i • i := by
            rw [Finset.smul_sum]; congr 1; funext i
            simp only [w', smul_smul]; field_simp
          rw [sum_eq]
          have mem_a := hmem' a (Finset.mem_insert_self a t)
          have sum_in_hull : ∑ i ∈ t, w' i • i ∈ (convexHull ℝ) ↑u' := by
            by_cases ht' : t = ∅
            · simp [ht'] at hw'_sum
            · exact (convex_convexHull ℝ (↑u' : Set α)).sum_mem hw'_pos hw'_sum hmem'_t
          calc f (w a • a + (1 - w a) • ∑ i ∈ t, w' i • i)
  ...
  calc
    _ ≤ (∑ j ∈ u', w j)⁻¹ * (∑ i ∈ u', w i * f i) := key
    _ ≤ (∑ j ∈ u', w j)⁻¹ * (∑ i ∈ u', w i * Finset.sup' u' u_nonempty f) := by
...
theorem FiniteDimensionalConvexFunctionsLocallyLipschitz
    (hs_convex : Convex ℝ s) (hs_isopen : IsOpen s)
...
theorem FiniteDimensionalConvexFunctionsContinous
    (hs_convex : Convex ℝ s) (hs_isopen : IsOpen s)
    (hf : ConvexOn ℝ s f) : ContinuousOn f s := by
