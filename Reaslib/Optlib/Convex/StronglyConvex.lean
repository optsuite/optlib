...
lemma strongconvex_of_convex_add_sq (f : E → ℝ) (x : E) (hfun : ConvexOn ℝ univ f) :
  ...
  · intro y yin z zin a b anneg bnneg absum1; simp
    ...
    · field_simp; rw [mul_sub, add_sub, ← mul_assoc]
      ...
      have hab : a = 1 - b := by exact eq_sub_of_add_eq absum1
      have eq2 (u v : E) : ‖a • u + b • v‖ ^ 2 = b * ‖v‖ ^ 2
        + a * ‖u‖ ^ 2 - a * b * ‖u - v‖ ^ 2 := by
        rw [norm_add_sq_real, norm_sub_sq_real]
        rw [inner_smul_left, inner_smul_right, norm_smul, norm_smul, hab]; field_simp
      ...
      let v := z - x
      rw [eq1, eq3]
      suffices ‖a • u + b • v‖ ^ 2 ≤ b * ‖v‖ ^ 2 + a * ‖u‖ ^ 2 - a * b * ‖u - v‖ ^ 2 by exact this
      apply eq4 u v
...
theorem Strong_Convex_second_lower (hsc : StrongConvexOn s m f)
  ...
  rw [sub_add, ← mul_sub, ← inner_sub_left, ← sub_sub,
    sub_right_comm,← add_sub x, add_sub_cancel] at foc
