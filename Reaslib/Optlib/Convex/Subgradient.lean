...
theorem SubderivAt.convex : ∀ x, Convex ℝ (SubderivAt f x) := by
  ...
  have ineq2 : b • f y ≥ b • f x + b • inner (ℝ) g₂ (y - x) := by
    rw [← smul_add]
    apply smul_le_smul_of_nonneg_left (h2 y) leb
  have eq : (a • f x + a • inner (ℝ) g₁ (y - x)) + (b • f x + b • inner (ℝ) g₂ (y - x))
      = f x + inner (ℝ) (a • g₁ + b • g₂) (y - x) := by
...
theorem subgradientAt_mono {u v : E} {f : E → ℝ} {y : E}
  ...
  have ineq1 : ⟪u, x - y⟫ ≥ f x - f y := by
    rw [congrArg (inner (ℝ) u) (Eq.symm (neg_sub y x)), inner_neg_right]; linarith
  have _ : inner (ℝ) v (x - y) ≤ f x - f y := Iff.mpr le_sub_iff_add_le' hv
...
theorem SubderivWithinAt_eq_gradient {f'x : E} (hx : x ∈ interior s)
  ...
  have mems : x + t • v ∈ s := by
    ...
    have : ‖t‖ * ‖v‖ < ε * ‖v‖⁻¹ * ‖v‖ := by
      apply (mul_lt_mul_iff_left₀ (norm_sub_pos_iff.mpr neq)).mpr tball
...
theorem first_order_convex_iff_subgradient (f : E → ℝ) (xm : E) :
    IsMinOn f univ xm ↔ 0 ∈ SubderivAt f xm := by
...
theorem HasSubgradientAt.pos_smul {c : ℝ} (h : HasSubgradientAt f g x) (hc : 0 < c) :
  ...
  have ineq : c * f y ≥ c * (f x + inner (ℝ) g (y - x)) := (mul_le_mul_iff_right₀ hc).mpr (h y)
  have eq : c * (f x + inner (ℝ) g (y - x)) = c * f x + c * inner (ℝ) g (y - x) :=
    mul_add c (f x) (inner (ℝ) g (y - x))
  ...
  · intro hg; use (c⁻¹ • g)
    ...
    · intro y;
      ...
      calc
        ...
        _ ≥ c⁻¹ * (c * f x + inner (ℝ) g (y - x)) :=
          mul_le_mul_of_nonneg_left (hg y) (inv_nonneg.mpr (le_of_lt hc))
        _ = f x + inner (ℝ) (c⁻¹ • g) (y - x) := by
          rw [mul_add, inner_smul_left, ← ((eq_inv_mul_iff_mul_eq₀ neq).mpr rfl)]
          rfl
    exact smul_inv_smul₀ (ne_of_gt hc) g
  rintro ⟨gg, hgg, eq⟩; intro y
  calc
    c * f y ≥ c * (f x + inner (ℝ) gg (y - x)) := (mul_le_mul_iff_right₀ hc).mpr (hgg y)
    _ = c * f x + c * inner (ℝ) gg (y - x) := mul_add c (f x) (inner (ℝ) gg (y - x))
    _ = c * f x + inner (ℝ) (c • gg) (y - x) := by
      rw [inner_smul_left]; exact rfl
    _ = c * f x + inner (ℝ) g (y - x) := by rw [← eq]
...
theorem SubderivAt.add {f₁ f₂ : E → ℝ} (h₁ : ConvexOn ℝ univ f₁) (h₂ : ConvexOn ℝ univ f₂)
  ...
  let S₁ := {(x, y) : E × ℝ | y > f₁ (x + x₀) - f₁ x₀ - inner (ℝ) g x}
  ...
  have hso : IsOpen S₁ := by
    apply Continuous_epi_open (f₁ := fun x ↦ f₁ (x + x₀) - f₁ x₀ - inner (ℝ) g x)
  ...
  have eq : ∃ a : E, ∃ b : ℝ, ∀ (p : E × ℝ), f p = inner (ℝ) a p.1 + b * p.2 := by
    ...
    intro p
    have : ((toDual ℝ ℝ).symm f2) * p.2 = inner (ℝ) (((toDual ℝ ℝ).symm f2)) p.2 := by
      simp [f2]
      exact
        CommMonoid.mul_comm
          ((toDual ℝ ℝ).symm (ContinuousLinearMap.comp f (ContinuousLinearMap.inr ℝ E ℝ))) p.2
    have : ((toDual ℝ ℝ).symm f2) * p.2 = f2 p.2 := by
      rw [this]
      simp only [toDual_symm_apply]
    rw [this]; simp [f1, f2]
  ...
  have g2 : - hata ∈ {g | HasSubgradientAt f₂ g x₀} := by
    ...
    linarith
  use (g + hata); constructor
  · exact g1
  use (- hata); constructor
  · exact g2
  simp
...
theorem SubderivAt_of_norm_at_zero : SubderivAt (fun (x : E) => ‖x‖) 0 = {g | ‖g‖ ≤ 1} := by
  ...
  intro hg y
  calc
    ‖(0 : E)‖ + inner (ℝ) g (y - 0) = inner (ℝ) g y := by simp only [norm_zero, zero_add, sub_zero]
...
theorem SubderivAt_abs (x : ℝ) :
  ...
  · intro hg
        ...
        have ineq : (0 : ℝ) < 0 := by
          calc
            0 ≥ x + g * (-x):= by
              ...
              have : inner (ℝ) g (-x) = g * (-x) := by
                simp only [RCLike.inner_apply, conj_trivial, neg_mul, mul_neg, neg_inj]
                exact mul_comm x g
              rwa [this] at hg
            _ = x * (1 - g) := by ring
    ...
    · specialize hg (x - 1)
      ...
      have : -g ≤ 1 := by
        calc
          -g = inner (ℝ) g (x - 1 - x) := by simp
          _ ≤ 1 := by linarith [hg]
      linarith
    specialize hg 0
    have eq1 : inner (ℝ) g (-x) = g * (-x) := by
      simp only [RCLike.inner_apply, conj_trivial, neg_mul, mul_neg, neg_inj]
      exact mul_comm x g
  ...
  · simp only [Real.sign_of_pos hx] at hg
    calc
      |x| + inner (ℝ) g (y - x) = x + inner (ℝ) 1 (y - x) := by rw [abs_of_pos hx, hg]
      _ = y := by simp
      _ ≤ |y| := le_abs_self y
  have hx : x < 0 := Ne.lt_of_le h (not_lt.mp hx)
  simp only [Real.sign_of_neg hx] at hg
  calc
    |x| + inner (ℝ) g (y - x) = -x + inner (ℝ) (-1) (y - x) := by rw [abs_of_neg hx, hg]
    _ = -y := by simp; ring
    _ ≤ |y| := neg_le_abs y
