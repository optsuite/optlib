...
theorem HasGradientAt.one_div (hf : HasGradientAt f grad x) (h₁ : ¬ f x = (0 : ℝ)) :
  ...
  intro ε εpos
  rcases (ContinuousAt_Convergence (ContinuousAt.abs
    (HasGradientAt.continuousAt hf))) ε εpos with ⟨δ', _ , _⟩
  ...
  have h₅ : ∃ δ > (0 : ℝ ), ∀ (x' : E), ‖x - x'‖ ≤ δ →
    ...
    have zp1 :‖f x * (f x - f x' + inner ℝ grad (x' - x)) / (f x * f x * f x')‖ =
        ‖(f x - f x' + inner ℝ grad (x' - x)) / (f x * f x')‖ := by
      rw [mul_comm, mul_assoc (f x) (f x) (f x'),
      div_mul_eq_div_div ((f x - f x' + inner ℝ
        grad (x' - x)) * (f x)) (f x) (f x * f x'), mul_div_cancel_right₀]
      apply h₁
    ...
    have zp3 : ‖1/(f x * f x')‖ ≤ 2 / (‖f x‖ * ‖f x‖) := by
      ...
      have h'' : 0 < ‖f x‖/2 := by
        ...
        norm_num
      apply lt_of_lt_of_le h'' h'
    have zp5 : ((ε * ‖f x‖ * ‖f x‖ / 4) * ‖x - x'‖) *
        (2 / (‖f x‖ * ‖f x‖)) = (ε / 2) * ‖x' - x‖ := by
      have l : ‖f x‖ ≠ 0 := by
    ...
    calc
      ‖f x * (f x - f x' + inner ℝ grad (x' - x)) / (f x * f x * f x')‖ =
          ‖(f x - f x' + inner ℝ grad (x' - x)) / (f x * f x')‖ := by
        apply zp1
      _ = ‖(f x - f x' + inner ℝ grad (x' - x))‖ * ‖1/(f x * f x')‖ := by
        apply Vert_div
      _ ≤ ‖(f x - f x' + inner ℝ grad (x' - x))‖ * (2 / (‖f x‖ * ‖f x‖)) := by
        apply mul_le_mul_of_nonneg_left zp3
        apply norm_nonneg
      _ ≤ ((ε * ‖f x‖ * ‖f x‖/4) * ‖x - x'‖) * (2 / (‖f x‖ * ‖f x‖)) := by
        have : ‖(f x - f x' + inner ℝ grad (x' - x))‖ ≤ (ε * ‖f x‖ * ‖f x‖/4) * ‖x - x'‖ := by
          apply hδ₂
          apply hp₂
  ...
  use min δ₀ (min δ₄ δ₅)
  constructor
  · apply lt_min δ₀pos (lt_min δ₄pos δ₅pos)
  intro x' h'
  have hp₀ : ‖x - x'‖ ≤ δ₀ := by
  ...
  have k₆ : (f x - f x') * f x /(f x' * f x * f x) + f x' * (⟪grad, (x' - x)⟫)/(f x' * f x * f x)
    ...
    apply (add_div ((f x - f x') * f x) (f x' * (⟪grad, (x' - x)⟫)) (f x' * f x * f x)).symm
  have k₇ : ((f x - f x') * f x + f x' * inner ℝ grad (x' - x)) / (f x' * f x * f x) =
      (f x * (f x - f x' + inner ℝ grad (x' - x)) +
      (f x' * inner ℝ grad (x' - x) - f x * inner ℝ grad (x' - x))) / (f x' * f x * f x) := by
    have h' : (f x - f x') * f x + f x' * inner ℝ grad (x' - x) =
        f x * (f x - f x' + inner ℝ grad (x' - x)) +
        (f x' * (inner ℝ grad (x' - x)) - f x * (inner ℝ grad (x' - x)))  := by
      linarith
    rw [h']
  have k₈ : (f x * (f x - f x' + inner ℝ grad (x' - x)) +
      (f x' * inner ℝ grad (x' - x) - f x * inner ℝ grad (x' - x))) /
      (f x * f x * f x') = f x * (f x - f x' + inner ℝ grad (x' - x))/
      (f x * f x * f x') + (f x' * inner ℝ grad (x' - x) - f x * inner ℝ grad (x' - x))/
      (f x * f x * f x') := by
        apply add_div ((f x) * (f x - f x' + inner ℝ grad (x' - x)))
          (f x' * inner ℝ grad (x' - x) - f x * inner ℝ grad (x' - x)) (f x * f x * f x')
  ...
  have p₁ : ‖1 / f x' - 1 / f x - (- (⟪grad, (x' - x)⟫))/((f x) * (f x))‖ ≤ ε * ‖x' - x‖ := by
    ...
    calc
      ...
      (f x * f x * f x')‖ := by
        rw [k₈]
      _ ≤ ‖f x * (f x - f x' + inner ℝ grad (x' - x))/
      (f x * f x * f x')‖ + ‖(f x' * inner ℝ grad (x' - x) - f x * inner ℝ grad (x' - x))/
      (f x * f x * f x')‖ := by
        apply norm_add_le ((f x) * (f x - f x' + inner ℝ grad (x' - x))/
      (f x * f x * f x')) ((f x' * inner ℝ grad (x' - x) - f x * inner ℝ grad (x' - x))/
      (f x * f x * f x'))
      _ ≤ (ε/2) * ‖x' - x‖ + (ε/2) * ‖x' - x‖ := by exact add_le_add (hδ₅ x' hp₂) (hδ₄ x' hp₁)
      _ = ε * ‖x' - x‖ := by linarith
  have j₁ : ‖1 / f x' - 1 / f x - (- (⟪grad, (x' - x)⟫))/((f x) * (f x))‖ = ‖1 / f x' - 1 / f x -
      inner ℝ ((-(1 / f x ^ ↑2) • grad)) (x' - x)‖ := by
    congr; rw [k₃]
  rw [j₁] at p₁
  have l1 : ‖x - x'‖ = ‖x' - x‖ := by
