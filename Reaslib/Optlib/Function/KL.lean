...
def desingularizing_function (η : ℝ) :=
  {φ : ℝ → ℝ | (ConcaveOn ℝ (Ico 0 η) φ) -- ∧ (∀ x ∈ Ioo 0 η, φ x > 0)
...
def KL_point (f : E → ℝ) (u : E) : Prop :=
  ∃ η ∈ Ioi 0, ∃ s ∈ 𝓝 u, ∃ φ ∈ desingularizing_function η, ∀ x ∈ s ∩
  {y | f u < f y ∧ f y < f u + η},
  (ENNReal.ofReal (deriv φ (f x - f u))) *
  (EMetric.infEdist 0 (subdifferential f x)) ≥ ENNReal.ofReal 1
...
def KL_property_with_regularization (f : E → ℝ) (u' : E) (φ : ℝ → ℝ) : Prop :=
  ...
  (EMetric.infEdist 0 (subdifferential (fun u => φ (f u - f u')) x)).toReal ≥ 1)
...
lemma const_mul_special_concave : ∀ c > 0,
    (fun t => c⁻¹ * t) ∈ desingularizing_function (c / 2) := by
  ...
  have h₁: ConcaveOn ℝ (Ico 0 (c / 2)) φ := by
    ...
    suffices a • (c⁻¹ * x) + b • (c⁻¹ * y) ≤ c⁻¹ *  (a • x + b • y) by exact this
    ...
    have h_contra : 0 ∈ subdifferential f x := by
      ...
      have v_to_zero: Tendsto v atTop (𝓝 0) := by
        have : Tendsto (fun n => ‖v n‖) atTop (𝓝 0) := by
          ...
          intro n
          apply le_of_lt
          obtain hn1 := (hv n).right
          rwa [dist_zero_left] at hn1
        apply tendsto_zero_iff_norm_tendsto_zero.2 this
      suffices (x, 0) ∈ subdifferential_Graph f by exact this
...
theorem uniformized_KL_property {f : E → ℝ} {Ω : Set E} (h_compact : IsCompact Ω)
    ...
    have : ∀ x ∈ Ω, ∃ η ∈ Ioi 0, ∃ (O : Set E) (_: IsOpen O) (_: x ∈ O),
        ∃ φ ∈ desingularizing_function η, ∀ u ∈ O ∩ {y | f x < f y ∧ f y < f x + η},
    ...
    have h_special_concave: φ_sum ∈ desingularizing_function η_min := by
      ...
      have h₁ : ConcaveOn ℝ (Ico 0 η_min) φ_sum := by
        ...
        have : ∀ d : ℝ, ∀ x : ℝ, d * ∑ c ∈ ht2.toFinset,
          ...
          intro d x
          let f c := φ c x
          suffices d * ∑ c ∈ ht2.toFinset, f c = ∑ c ∈ ht2.toFinset, d * f c by exact this
      ...
      have h₆ : ∀ (x : ℝ), 0 < x → x < η_min → 0 < deriv φ_sum x := by
        ...
        · exact t_nonempty
      exact ⟨h₁,h₃,h₄,h₅,h₆⟩
    have uniform_ball: ∃ ε ∈ Ioi 0, {y| EMetric.infEdist y Ω <
        ENNReal.ofReal ε} ⊆ ⋃ x ∈ t, O x := by
    ...
    have : {y| EMetric.infEdist y Ω < ENNReal.ofReal ε}
        = {y| (EMetric.infEdist y Ω).toReal < ε} := by
