...
This file contains several key definitions and theorems
  that involve continuous linear maps and their properties in this space.
...
theorem sigma_norm_apply : ∀ x, ∀ j, ∑ i,
    (((Module.finBasis ℝ α).repr x) i) • σ ((Module.finBasis ℝ α) i) j
...
in the finite basis representation of x,
each multiplied by the norm of the corresponding basis vector.
-/
theorem l1_norm_eq : ∀ x , ‖σ x‖ =  ∑ i,
    ‖((Module.finBasis ℝ α).repr x) i‖ * ‖(Module.finBasis ℝ α) i‖ := by
  intro x
...
theorem l1Ball_sub_convexHull {x : α} {r : ℝ} (hr : r > 0) (hn : Module.finrank ℝ α ≠ 0) :
    σ.toFun ⁻¹' (Metric.ball (σ.toFun x) r) ⊆ convexHull ℝ (({x} : Set α) + ((⋃ i ,
    {(r / ‖b i‖) • (b i)})  ∪  (⋃ i ,{- (r / ‖b i‖) • (b i)}))):= by
  intro x₀ hx₀
  ...
  have hz : ∀ (i : ι₀), z i ∈ ((⋃ i , {(r / ‖b i‖) • (b i)})
    ∪ (⋃ i ,{- (r / ‖b i‖) • (b i)})) := by
    intro ⟨i, hi⟩
    simp only [Set.mem_union, Set.mem_iUnion, Set.mem_singleton_iff]
    -- Case split on the index i
    by_cases h₁ : i = n + 1
    · -- Case: i = n + 1, then z i = -(r / ‖b fin0‖) • (b fin0)
      right
      use fin0
      simp only [z, h₁, dif_pos]
    by_cases h₂ : i = n
    · -- Case: i = n, then z i = (r / ‖b fin0‖) • (b fin0)
      left
      use fin0
      simp only [z, h₂, dif_pos]
      have : ¬ (n = n + 1) := by omega
    ...
    by_cases h₃ : (b).equivFun (x₀ - x) use_i = 0
    · -- Subcase: coefficient is zero, take positive version
      left
      use use_i
      simp only [z, h₁, h₂, dif_neg (not_false)]
      rw [dif_pos h₃]
    · -- Subcase: coefficient is nonzero, check its sign
      rcases lt_trichotomy ((b).equivFun (x₀ - x) use_i) 0 with ha | ha | ha
      · -- coefficient < 0, so sign is negative
        right
        use use_i
        simp only [z, h₁, h₂, dif_neg (not_false)]
        rw [dif_neg h₃]
        have repr_eq : (b).equivFun (x₀ - x) use_i = (b).repr (x₀ - x) use_i := by
          simp only [Module.Basis.equivFun_apply]
        have h_sub : (b).repr (x₀ - x) use_i < 0 := by rw [← repr_eq]; exact ha
        have sign_eq : (SignType.sign ((b).equivFun (x₀ - x) use_i) : ℝ) = -1 := by
          simp only [SignType.sign, OrderHom.coe_mk, repr_eq, map_sub]
          simp only [map_sub] at h_sub
          -- The nested if simplifies because h_sub < 0
        ...
        rfl
      · -- coefficient = 0 (contradiction with h₃)
        exfalso
        exact h₃ ha
      · -- coefficient > 0, so sign is positive
        left
        use use_i
        simp only [z, h₁, h₂, dif_neg (not_false)]
        rw [dif_neg h₃]
        have repr_eq : (b).equivFun (x₀ - x) use_i = (b).repr (x₀ - x) use_i := by
          simp only [Module.Basis.equivFun_apply]
        have h_sub : 0 < (b).repr (x₀ - x) use_i := by rw [← repr_eq]; exact ha
        have sign_eq : (SignType.sign ((b).equivFun (x₀ - x) use_i) : ℝ) = 1 := by
          simp only [SignType.sign, OrderHom.coe_mk, repr_eq, map_sub]
          simp only [map_sub] at h_sub
          -- The nested if simplifies because h_sub > 0
  ...
  have hx : ∑ i : ι₀, w i • z i = x₀ - x := by
    ...
    have : w₁ i • ((SignType.sign ((b).equivFun (x₀ - x) i)) * (r / ‖b i‖)) = w₀ i := by
      ...
      calc
        _ = |(b).equivFun (x₀ - x) i| * (‖b i‖ / r) *
          (SignType.sign ((b).equivFun (x₀ - x) i)) * (r / ‖b i‖) := by
          rw[← mul_div]
          linarith
        _ = ((SignType.sign ((b).equivFun (x₀ - x) i)) *
          |(b).equivFun (x₀ - x) i|) * ((‖b i‖ / r) * (r / ‖b i‖)) := by
          linarith
        _ = _ :=by
          rw[sign_mul_abs]
          field_simp [bi_pos i]
