  ...
  have eq : f xm ≤ f x - 1 / (2 * l) * ‖f' x‖ ^ 2 := by
    calc
      ...
      _ = f x - 1 / (2 * l) * ‖f' x‖ ^ 2 := by
        ...
        simp only [y, sub_sub_cancel_left, inner_neg_right, norm_neg,
          div_mul_eq_mul_div, one_mul, neg_div']
        rw [← real_inner_self_eq_norm_sq, ← real_inner_self_eq_norm_sq]
        rw [inner_smul_right, inner_smul_left, inner_smul_right]
        simp only [div_mul_eq_mul_div, one_mul, neg_div', map_div₀,
          conj_trivial, div_div, mul_div_assoc']
        field_simp; ring
  linarith
