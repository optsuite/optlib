...
lemma empty_FeasSet_inf_value_top {p : Constrained_OptimizationProblem E τ σ}
    (hp : (p.FeasSet) = ∅) : p.inf_value = ⊤ := by
  unfold inf_value; simp [hp]

lemma empty_domain_sup_value_bot {p : Constrained_OptimizationProblem E τ σ} (hp : (p.domain) = ∅) :
    p.sup_value = ⊥ := by
  unfold sup_value
  unfold FeasSet FeasPoint
  simp [hp]

lemma empty_FeasSet_sup_value_bot {p : Constrained_OptimizationProblem E τ σ}
    (hp : (p.FeasSet) = ∅) : p.sup_value = ⊥ := by
  unfold sup_value; simp [hp]
...
lemma dual_objective_le_top_nonempty {p : Constrained_OptimizationProblem E τ σ}
    (hp : (p.domain).Nonempty) :
...
lemma dual_objective_eq_top_empty {p : Constrained_OptimizationProblem E τ σ}
    (hp : (p.domain) = ∅) :
    ∀ lambda1 lambda2, p.dual_objective lambda1 lambda2 = ⊤ := by
...
lemma global_minimum_objective_infimum {p : Constrained_OptimizationProblem E τ σ}
    (hp : p.Global_Minimum x) : (p.objective x).toEReal = p.inf_value := by
...
lemma ConcaveOn.sum {𝕜 : Type*} [Semiring 𝕜] [PartialOrder 𝕜]
    [IsOrderedRing 𝕜] [AddCommMonoid α] [SMul 𝕜 α]
...
theorem convex_problem_convex_Lagrange {p : Constrained_OptimizationProblem E τ σ}
  ...
  · apply ConvexOn.sub h
    simp only [hτ, Finset.notMem_empty, isEmpty_subtype, not_false_eq_true, implies_true,
...
theorem optimal_multipliers_solution_dual_problem {p : Constrained_OptimizationProblem E τ σ}
  ...
  · obtain h1 := KKT_multipliers_inf_value h hτ x hf hi conti lambda1 lambda2 hg hKKT
    ...
    have : (p.dual_problem).objective (lambda1, lambda2) ≤ (p.dual_problem).sup_value := by
      apply objective_le_sup _
        (KKT_multipliers_dual_feasible h hτ x hf hi conti lambda1 lambda2 hg hKKT)
    rw [← h1] at h2
